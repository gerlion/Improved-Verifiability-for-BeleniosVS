...
# We refine the threat model so that #IFDVOTERCHAN is no longer a single flag
...
$all .= $honestvoter =~ s/\$vote/a/gr =~ s/\$VOTE/A/gr;
$all .= $honestvoter =~ s/\$vote/b/gr =~ s/\$VOTE/B/gr;
...
$all .= readfile("main.pv");

my $fileout="generated-files/r".$registrar." a".$audit." d".$votingdevice." s".$votingserver." vs".$votingsheet." pwd".$password.".pv";
